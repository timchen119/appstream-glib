 ...
 * @unique_id: a unique application ID, e.g.
 *             "user/flatpak/gnome-apps-nightly/app/gimp.desktop/i386/master"
 *
 * Verifies the unique ID is valid. A valid ID always has 7 components, but
 * any but the first item can be blank.
 *
...
/**
 * as_utils_unique_id_split:
 * @unique_id: a unique application ID, e.g.
 *             "user/flatpak/gnome-apps-nightly/app/gimp.desktop/i386/master"
 *
 * Splits the unique ID into parts.
 *
 * Returns: the data, or %NULL if invalid
 */
gchar **
as_utils_unique_id_split (const gchar *unique_id)
{
	g_auto(GStrv) split = NULL;
 ...
 * @unique_id1: a unique application ID
 * @unique_id2: another unique application ID
 *
 * Compares each ID for equality, taking into account wildcards.
 *
...
as_utils_unique_id_equal (const gchar *unique_id1, const gchar *unique_id2)
{
	...
	/* trivial */
	if (g_strcmp0 (unique_id1, unique_id2) == 0)
		return TRUE;

	/* compare sections */
	split1 = as_utils_unique_id_split (unique_id1);
	if (split1 == NULL)
		return FALSE;
	split2 = as_utils_unique_id_split (unique_id2);
	if (split2 == NULL)
		return FALSE;
	for (i = 0; split1[i] != NULL && split2[i] != NULL; i++) {
		if (g_strcmp0 (split1[i], "*") == 0 ||
		    g_strcmp0 (split2[i], "*") == 0)
			continue;
 ...
 * @scope: Scope e.g. 'system' or 'user'
 * @system: System, e.g. 'package' or 'flatpak'
 * @origin: Origin, e.g. 'fedora' or 'gnome-apps-nightly'
 * @kind: Kind, e.g. 'app' or 'runtime'
 ...
 */
gchar *
as_utils_unique_id_build (const gchar *scope,
			  const gchar *system,
			  const gchar *origin,
			  const gchar *kind,
			  const gchar *id,
			  ...
			  const gchar *branch)
{
	...
	return g_strdup_printf ("%s/%s/%s/%s/%s/%s/%s",
				_as_utils_fix_unique_id_part (scope),
				_as_utils_fix_unique_id_part (system),
				_as_utils_fix_unique_id_part (origin),
				_as_utils_fix_unique_id_part (kind),
				id,
